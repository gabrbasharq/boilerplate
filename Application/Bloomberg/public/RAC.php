...
if(! defined('FILE_CACHE_TIME_BETWEEN_CLEANS'))	define ('FILE_CACHE_TIME_BETWEEN_CLEANS', 8.64e+8);	// How often the cache is cleaned

if(! defined('FILE_CACHE_MAX_FILE_AGE') ) 	define ('FILE_CACHE_MAX_FILE_AGE', 8.64e+7);				// How old does a file have to be to be deleted from the cache
if(! defined('FILE_CACHE_SUFFIX') ) 		define ('FILE_CACHE_SUFFIX', '.faris.txt');			// What to put at the end of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_PREFIX') ) 		define ('FILE_CACHE_PREFIX', 'faris');				// What to put at the beg of all files in the cache directory so we can identify them
...
if(! defined('NOT_FOUND_IMAGE') )		define ('NOT_FOUND_IMAGE', './notFoundImage.jpg');								// Image to serve if any 404 occurs
if(! defined('ERROR_IMAGE') )			define ('ERROR_IMAGE', './notFoundImage.jpg');									// Image to serve if an error occurs instead of showing error message
...
class timthumb {
	protected $originalImagesPath = '/uploads/general_images/low_original/';
	protected $src = "";
	...
	public function __construct(){
		...
		$this->src = $this->originalImagesPath . $this->param('src');
//		echo $this->src;exit;
	...
	protected function processImageAndWriteToCache($localImage){
		...
		if ($zoom_crop > 0) {
			...
			if ($align && ! $x && ! $y) {
                ...
                if ($cmp_x > $cmp_y) {
                    $src_w = round ($width / $cmp_x * $cmp_y);
                    $src_x = round (($width - ($width / $cmp_x * $cmp_y)) / (100 / $x));

                } else if ($cmp_y > $cmp_x) {
                    $src_h = round ($height / $cmp_y * $cmp_x);
                    $src_y = round (($height - ($height / $cmp_y * $cmp_x)) / (100 / $y));
                }
            }
//            $src_y =  $cmp_y / $y;
//            $src_x =  $src_x - $x ;
...
//			echo $src_y;exit;
	...
	protected function sendImageHeaders($mimeType, $dataSize){
        ...
        header('Content-Disposition: filename=Qmxvb21iZXJnIEltYWdlIChGLkEp_' . @$_GET['w'] . 'x' . @$_GET['h'] .  '_' .  @$_GET['src']);
